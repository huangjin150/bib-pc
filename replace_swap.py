...
with open(r'c:\frontend\bib-pc\src\views\swap\SwapIndex.vue', 'r', encoding='utf-8') as f:
    content = f.read()

start_marker = r'      <!-- 成交记录 -->'
end_marker = r'    <!-- 弹出框: 变更仓位模式 -->'
...
pattern = re.compile(start_marker + r'.*?    </div>\n    </div>\n\n\n    ' + end_marker, re.DOTALL)
new_content = pattern.sub(new_block + "\n\n    " + end_marker, content)

with open(r'c:\frontend\bib-pc\src\views\swap\SwapIndex.vue', 'w', encoding='utf-8') as f:
    f.write(new_content)
