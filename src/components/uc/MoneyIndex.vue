<template>
  <div class="nav-rights">
    ...
    <!-- <Modal v-model="rechargeBsc" width="360">
		<RechargeBsc :rechargeToken="rechargeToken" ref="rechargeModel"></RechargeBsc>
		<div slot="footer">
		    <Button size="large" @click="rechargeBsc = false">{{$t("common.close")}}</Button>
		    <Button type="primary" size="large" @click="confirmRecharge">{{$t("uc.finance.withdraw.submit")}}</Button>
		</div>
	</Modal>
	<Modal v-model="withdrawBsc" width="360">
		<WithdrawBsc :withdrawToken="withdrawToken" :balance="balance" ref="withdrawModel" @overWithdraw="overWithdraw()"></WithdrawBsc>
		<div slot="footer">
		    <Button size="large" @click="withdrawBsc = false">{{$t("common.close")}}</Button>
		    <Button type="primary" size="large" @click="confirmWithdraw">{{$t("uc.finance.withdraw.submit")}}</Button>
		</div>
	</Modal> -->
  ...
  computed: {
    totalUSDT() {
      ...
      for (let i = 0; i < this.tableMoney.length; i++) {
        ...
        console.log("this.tableMoney[i].balance", this.tableMoney[i].balance)
      }
