      ...
      <div class="padding-right-clear padding-left-clear rightarea user account-box">
        <div class="rightarea-con">
          <div class="user-top-icon">
            ...
            <div class="user_memberLevel" v-if="user.memberLevel !== 0">
              <div class="user_memberLevel_i"><img :src="user_memberLevel" alt=""></div>
              <div>{{ $t('uc.safe.memberLevel') }}</div>
              <div>V{{ user.memberLevel }}</div>
            </div>
          </div>
            ...
            <div class="account-in">
              ...
              <div class="account-item" style="display: none;">
                <div class="account-item-in">
                  ...
                  <span @click="showItem(6)" v-if="user.realVerified == 1">{{ $t('uc.safe.verifypass') }}</span>
                  ...
                  <div class="detail-list" style="width: 100%;">
                    <Form ref="formValidate6" :model="formValidate6" :rules="ruleValidate" :label-width="85"
                      ...
                      <FormItem style="text-align:center;">
                        <Button type="warning" @click="handleSubmit('formValidate6')" style="margin-left: -85px;">{{
                          $t('uc.safe.save') }}</Button>
                        <Button @click="handleReset('formValidate6')" style="margin-left: 8px">{{ $t('uc.safe.reset')
                        }}</Button>
                      </FormItem>
                    </Form>
                  </div>
                  ...
                  <div class="detail-list">
                    <Form ref="formValidate3" :model="formValidate3" :rules="ruleValidate" :label-width="110">
                      ...
                      <FormItem>
                        ...
                        <Button @click="handleReset('formValidate3')" style="margin-left: 8px">{{ $t('uc.safe.reset')
                        }}</Button>
                      </FormItem>
                    </Form>
                  </div>
                  ...
                  <div class="detail-list">
                    <Form ref="formValidate2" :model="formValidate2" :rules="ruleValidate" :label-width="110">
                      ...
                      <FormItem>
                        ...
                        <Button @click="handleReset('formValidate2')" style="margin-left: 8px">{{ $t('uc.safe.reset')
                        }}</Button>
                      </FormItem>
                    </Form>
                  </div>
                ...
                <div class="account-detail" v-show="choseItem == 4">
                  <div class="detail-list">
                    <Form ref="formValidate4" :model="formValidate4" :rules="ruleValidate" :label-width="95">
                      ...
                      <!-- 手机验证码 -->
                      <FormItem v-if="user.phoneVerified !== 0" :label="$t('uc.safe.phonecode')" prop="vailCode3">
                      ...
                      <FormItem v-else :label="$t('uc.safe.emailcode')" prop="vailCode3">
                        <Input v-model="formValidate4.vailCode3" size="large">
                        <div class="timebox" slot="append">
                          <Button @click="send(3)" :disabled="sendMsgDisabled3">
                            <span v-if="sendMsgDisabled3">{{ time3 + $t('uc.safe.second') }}</span>
                      ...
                      <FormItem>
                        ...
                        <Button @click="handleReset('formValidate4')" style="margin-left: 8px">{{ $t('uc.safe.reset')
                        }}</Button>
                      </FormItem>
                    </Form>
                  </div>
                ...
                <div class="account-detail" v-show="choseItem == 5">
                  ...
                  <div class="detail-list" v-show="user.fundsVerified != 1">
                    <Form ref="formValidate7" :model="formValidate7" :rules="ruleValidate" :label-width="85">
                      ...
                      <FormItem>
                        ...
                        <Button @click="handleReset('formValidate7')" style="margin-left: 8px">{{ $t('uc.safe.reset')
                        }}</Button>
                      </FormItem>
                    </Form>
                  </div>
                  ...
                  <div class="detail-list" v-show="user.fundsVerified == 1 && !fGetBackFundpwd">
                    <Form ref="formValidate5" :model="formValidate5" :rules="ruleValidate" :label-width="95">
                      ...
                      <FormItem>
                        ...
                        <Button @click="handleReset('formValidate5')" style="margin-left: 8px">{{ $t('uc.safe.reset')
                        }}</Button>
                      </FormItem>
                    </Form>
                  </div>
                  ...
                  <div class="detail-list" v-show="user.fundsVerified == 1 && fGetBackFundpwd">
                    <Form ref="formValidate8" :model="formValidate8" :rules="ruleValidate" :label-width="85">
                      ...
                      <!-- 邮箱验证码 -->
                      <FormItem v-if="user.phoneVerified !== 0" :label="$t('uc.safe.phonecode')" prop="vailCode5">
                        <Input v-model="formValidate8.vailCode5" size="large">
                        <div class="timebox" slot="append">
                          <Button @click="send(3)" :disabled="sendMsgDisabled5">
                            <span v-if="sendMsgDisabled5">{{ time5 + $t('uc.safe.second') }}</span>
                            <span v-if="!sendMsgDisabled5">{{ $t('uc.safe.clickget') }}</span>
                          </Button>
                        </div>
                      ...
                      <FormItem v-else :label="$t('uc.safe.emailcode')" prop="vailCode5">
                        <Input v-model="formValidate8.vailCode5" size="large">
                        <div class="timebox" slot="append">
                          <Button @click="send(3)" :disabled="sendMsgDisabled5">
                            <span v-if="sendMsgDisabled5">{{ time5 + $t('uc.safe.second') }}</span>
                      ...
                      <FormItem>
                        ...
                        <Button @click="handleReset('formValidate8')" style="margin-left: 8px">{{ $t('uc.safe.reset')
                        }}</Button>
                      </FormItem>
                    </Form>
                  </div>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      user_memberLevel: require("../../assets/images/memberLevel.png"),
      VerificationData: {
        geetest_challenge: '',
        geetest_seccode: '',
        geetest_validate: '',
  ...
  methods: {
    ...
    submit(name) {
      ...
      if (name == "formValidate6") {
        ...
        this.$http
          .post(this.host + "/approve/real/name", param)
          .then(response => {
      ...
      if (name == "formValidate3") {
        ...
        this.$http
          .post(this.host + "/approve/bind/phone", param)
          .then(response => {
            var resp = response.body;
            if (resp.code == 0) {
              this.$Message.success(this.$t("uc.safe.save_success"));
      ...
      if (name == "formValidate4") {
        ...
        param["verify"] = this.user.phoneVerified !== 0 ? '0' : '1'
        this.$http
      ...
      if (name == "formValidate5") {
        ...
        this.$http
          .post(this.host + "/approve/update/transaction/password", param)
          .then(response => {
            var resp = response.body;
            if (resp.code == 0) {
              this.$Message.success(this.$t("uc.safe.save_success"));
      ...
      if (name == "formValidate7") {
        ...
        this.$http
          .post(this.host + "/approve/transaction/password", param)
          .then(response => {
            var resp = response.body;
            if (resp.code == 0) {
              this.$Message.success(this.$t("uc.safe.save_success"));
      ...
      if (name == "formValidate8") {
        ...
        param["verify"] = this.user.phoneVerified !== 0 ? '0' : '1'
        this.$http
          .post(this.host + "/approve/reset/transaction/password", param)
          .then(response => {
            var resp = response.body;
            if (resp.code == 0) {
...
.safe .ivu-form-item-error .ivu-input-group-append,
.safe .ivu-form-item-error .ivu-input-group-prepend,
.safe .ivu-input-group-append,
.safe .ivu-input-group-prepend {
  border-top: 1px solid #27313e;
  border-left: 1px solid #27313e;
