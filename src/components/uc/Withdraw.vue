            ...
            <div class="action-inner">
              ...
              <div class="inner-box">
                <div class="form-group form-address">
                  <label for="controlAddress" class="controlAddress describe">{{ $t('uc.finance.withdraw.address')
                  }}</label>
                  <div class="control-input-group">
                    <Select ref="address" v-model="withdrawAdress" filterable clearable
                      @on-query-change="onAddressChange" :placeholder="$t('common.pleaseselect')">
                      <Option v-for="item in currentCoin.addresses" :value="item.id" :key="item.address">{{
                        item.remark + '(' + item.address + ')' }}</Option>
                    </Select>
                  </div>
                </div>
              </div>
              ...
              <div class="form-group form-amount">
                <label class="label-amount"> {{ $t('uc.finance.withdraw.num') }}
                  <p class="label-fr">
                    ...
                    <span v-if="currentCoin.enableAutoWithdraw == 0">【{{ $t('common.tip') }}】：{{
                      $t('uc.finance.withdraw.msg1') }}
                      {{ currentCoin.threshold }} {{ $t('uc.finance.withdraw.msg2') }}</span>
                    <span>
                      <a href="javascript:;" id="levelUp" style="display: none;">{{ $t('uc.finance.withdraw.increase')
                      }}</a>
                    </span>
                  </p>
                </label>
                <!---->
                <div style="width: 100%; position: relative; margin-top: 20px;" class="input-group">
                  <InputNumber @on-change="computerAmount" v-model="withdrawAmount"
                    :placeholder="$t('uc.finance.withdraw.numtip1')" size="large" :max="currentCoin.balance">
                  </InputNumber>
                  <span class="input-group-addon  positioning uppercase firstt">{{
                    currentCoin.unit }}</span>
                </div>
              </div>
            </div>
            <div class="form-group-container form-group-container2">
              ...
              <div class="form-group form-fee">
                <label class="label-amount"> {{ $t('uc.finance.withdraw.fee') }}
                  <span style="color: red;">
                    {{ currentCoin.maxTxFee }}
                  </span>{{ currentCoin.unit }}
                  <!--<p class="label-fr">-->
              ...
              <div class="form-group">
                <label>{{ $t('uc.finance.withdraw.arriamount') }}
                  <span style="color: aquamarine;">{{ withdrawOutAmount }}</span>
                  {{ currentCoin.unit }}
                </label>
              </div>
            </div>
            <div class="action-foot">
              <Button id="withdrawSubmit" long size="large" type="primary" style="height:40px;" @click="apply">{{
                $t('uc.finance.withdraw.pickup') }}</Button>
            </div>
            <div class="action-content pt10">
              <div class="action-body">
                <p class="acb-p1">{{ $t('common.tip') }}</p>
                <p class="acb-p2">• {{ $t('uc.finance.withdraw.msg3') }}：{{ currentCoin.minAmount }} {{ coinType
                }}。<br>•
              ...
              <div class="action-body">
                ...
                <div class="order-table">
                  <p class="acb-p2" style="margin-bottom:10px;">• {{ $t('uc.finance.withdraw.click') }}
                    <i class="ivu-icon ivu-icon-funnel"></i>{{ $t('uc.finance.withdraw.filtrate') }}
                  </p>
                  <Table :no-data-text="$t('common.nodata')" :columns="tableColumnsWithdraw" :data="tableWithdraw"
                    :loading="loading"></Table>
                  <div id="pages">
                    <div style="float: right;">
                      <Page class="pages_a" :total="transaction.total" :current="transaction.page"
...
export default {
  ...
  methods: {
    ...
    sendCode() {
      this.$http.post(this.host + "/mobile/withdraw/code").then(response => {
    ...
    ok() {
      ...
      if (this.withdrawAdress == '') {
        this.modal = true;
        this.$Message.error(this.$t("otc.chat.msgorder"));
        return;
      }
      let params = {};
    ...
    getList() {
      ...
      this.$http
        .post(this.host + "/withdraw/record", params)
        .then(response => {
          var resp = response.body;
          if (resp.code == 0) {
            this.tableWithdraw = resp.data.records;
            this.transaction.total = resp.data.total;
            this.transaction.page = resp.data.current;
          } else {
            this.$Message.error(resp.message);
          }
          this.loading = false;
    ...
    changeMetwoker(value) {
      ...
      for (var i = 0; i < this.netwoker.length; i++) {
        if (this.netwoker[i].name == value) {
          // this.addressData.address = this.netwoker[i].usdtAddress
          this.chains = this.netwoker[i].id
          console.log(this.netwoker[i].id)
