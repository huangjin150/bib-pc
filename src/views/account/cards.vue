    ...
    <div class="nav-rights">
        <div class="banner ">
            <div class=" banner-info flex">
                <div style="flex: 1;">
                    <div class="banner_title">卡券中心</div>
                </div>
                <div style="flex: 1 1 0%;">
                    <div class="banner-bg">
                        <img src="../../assets/svg/cardimg.svg" alt="">
                    </div>
                </div>
            </div>
        </div>
        <div class="container">
                ...
                <div class="bill_box rightarea padding-right-clear">
                    <div class="shaow">
                        <div class="money_table">
                            <div style="width: 100%;height: 50px; padding:  0 0 70px 0;">
                                <div class="header-btn" @click="exchangeCard">{{ $t('uc.promotion.exchangewithcode') }}
                                </div>
                            </div>
                            <Table :columns="tableColumnsCard" :data="tableCardList" :loading="loading"
            ...
            <Modal v-model="showModal" :title="this.$t('uc.promotion.inputcardno')" width="360">
                ...
                <div slot="footer">
                    <Button type="error" size="large" long :loading="modal_loading" @click="exchange">{{
        ...
        <Drawer :title="promotionTitle" :closable="false" v-model="showPromotionModal" width="350"
            ...
            <div style="position:relative;width: 318px;" id="promotionImage" ref="promotionImage">
                ...
                <p
                    ...
                    {{ promotionCode }}</p>
                <p style="position:absolute;top: 250px;text-align:center;width: 100%;text-align:center;">推广合伙人专属兑换码
                </p>
            </div>
            <p style="text-align:center;font-size:12px;color:#888;margin-top: 10px;">{{ $t('invite.imagetips') }}
            </p>
...
export default {
    ...
    methods: {
        getMyCardList() {
            this.$http.get(this.swapHost + this.api.uc.mycardlist).then(response => {
        ...
        exchange() {
            ...
            this.$http.post(this.swapHost + this.api.uc.exchangecard, param).then(response => {
                var resp = response.body;
                if (resp.code == 0) {
            ...
            columns.push({
                ...
                render(h, params) {
                    return h(
                        ...
                        {
                            attrs: {
                                title: params.row.used
                            }
                        },
                        self.toFloor(params.row.used || "0")
                    );
                }
            });
            columns.push({
...
.banner {
    ...
    .banner-bg {
        width: 70%;
        background-image: url('../../assets/images/bg-dark.36db97d0.png');
